...
3. **Group membership is contractual** (own 2). `hvac` belongs to group
   `thermal`. Do not silently reassign.
...
| Task | Where |
|---|---|
| Read the spec | [`hvac-system.md`](hvac-system.md) |
| Tweak a number | First update `../../core/energy/domain.md`, then mirror here |
| Wire this verb | Add `verify_hvac.{hexa,py}`, register from `../../verify/energy_verify.py`, propagate wired count to spec.md/hexa.toml/cli |
...
Extracted from `n6-architecture/domains/energy/hvac-system/` on 2026-05-06
(SHA `c0f1f570`). Moved from repo root into `module/hvac/` on 2026-05-07

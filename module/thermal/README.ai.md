# module/thermal — AI-native handoff
...
1. **Numerical SSOT is `../../core/energy/domain.md`** (own 4). If a
   number in [`thermal-management.md`](thermal-management.md) diverges from `domain.md`,
   reconcile by updating either side — do NOT leave the divergence.
2. **Verb roster is contractual** (own 1). Do not rename `thermal` or move
...
3. **Group membership is contractual** (own 2). `thermal` belongs to group
...
| Task | Where |
|---|---|
| Read the spec | [`thermal-management.md`](thermal-management.md) |
| Tweak a number | First update `../../core/energy/domain.md`, then mirror here |
| Wire this verb | Add `verify_thermal.{hexa,py}`, register from `../../verify/energy_verify.py`, propagate wired count to spec.md/hexa.toml/cli |

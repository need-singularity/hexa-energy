# module/pv_microgrid — AI-native handoff
...
- **Verb**: `pv_microgrid` (group: `grid`)
- **Parent (canonical SSOT)**: `../../core/energy/domain.md` (numerical SSOT, own 4)
- **Upstream**: `n6-architecture/domains/energy/rooftop-pv-2nd-life-microgrid/` @ `c0f1f570`
- **Status**: SPEC_FIRST (own 5) — markdown spec only at v1.0.0

## Edit invariants

1. **Numerical SSOT is `../../core/energy/domain.md`** (own 4). If a
   number in [`rooftop-pv-2nd-life-microgrid.md`](rooftop-pv-2nd-life-microgrid.md) diverges from `domain.md`,
   reconcile by updating either side — do NOT leave the divergence.
2. **Verb roster is contractual** (own 1). Do not rename `pv_microgrid` or move
...
3. **Group membership is contractual** (own 2). `pv_microgrid` belongs to group
...
| Task | Where |
|---|---|
| Read the spec | [`rooftop-pv-2nd-life-microgrid.md`](rooftop-pv-2nd-life-microgrid.md) |
| Tweak a number | First update `../../core/energy/domain.md`, then mirror here |
| Wire this verb | Add `verify_pv_microgrid.{hexa,py}`, register from `../../verify/energy_verify.py`, propagate wired count to spec.md/hexa.toml/cli |
...
Extracted from `n6-architecture/domains/energy/rooftop-pv-2nd-life-microgrid/` on 2026-05-06
(SHA `c0f1f570`). Moved from repo root into `module/pv_microgrid/` on 2026-05-07

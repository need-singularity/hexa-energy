# module/smr_dc — AI-native handoff
...
3. **Group membership is contractual** (own 2). `smr_dc` belongs to group
...
| Task | Where |
|---|---|
| Read the spec | [`smr-datacenter.md`](smr-datacenter.md) |
| Tweak a number | First update `../../core/energy/domain.md`, then mirror here |
| Wire this verb | Add `verify_smr_dc.{hexa,py}`, register from `../../verify/energy_verify.py`, propagate wired count to spec.md/hexa.toml/cli |
...
Extracted from `n6-architecture/domains/energy/smr-datacenter/` on 2026-05-06
(SHA `c0f1f570`). Moved from repo root into `module/smr_dc/` on 2026-05-07

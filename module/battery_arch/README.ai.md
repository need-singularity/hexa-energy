# module/battery_arch — AI-native handoff
...
- **Verb**: `battery_arch` (group: `battery`)
- **Parent (canonical SSOT)**: `../../core/energy/domain.md` (numerical SSOT, own 4)
- **Upstream**: `n6-architecture/domains/energy/battery-architecture/` @ `c0f1f570`
- **Status**: SPEC_FIRST (own 5) — markdown spec only at v1.0.0

## Edit invariants

1. **Numerical SSOT is `../../core/energy/domain.md`** (own 4). If a
   number in [`battery-architecture.md`](battery-architecture.md) diverges from `domain.md`,
   reconcile by updating either side — do NOT leave the divergence.
2. **Verb roster is contractual** (own 1). Do not rename `battery_arch` or move
...
3. **Group membership is contractual** (own 2). `battery_arch` belongs to group
   `battery`. Do not silently reassign.
...
| Task | Where |
|---|---|
| Read the spec | [`battery-architecture.md`](battery-architecture.md) |
| Tweak a number | First update `../../core/energy/domain.md`, then mirror here |
| Wire this verb | Add `verify_battery_arch.{hexa,py}`, register from `../../verify/energy_verify.py`, propagate wired count to spec.md/hexa.toml/cli |
...
Extracted from `n6-architecture/domains/energy/battery-architecture/` on 2026-05-06
(SHA `c0f1f570`). Moved from repo root into `module/battery_arch/` on 2026-05-07

# module/mineshaft — AI-native handoff
...
- **Verb**: `mineshaft` (group: `mining`)
- **Parent (canonical SSOT)**: `../../core/energy/domain.md` (numerical SSOT, own 4)
- **Upstream**: `n6-architecture/domains/energy/amd-ree-mineshaft-phes/` @ `c0f1f570`
- **Status**: SPEC_FIRST (own 5) — markdown spec only at v1.0.0

## Edit invariants

1. **Numerical SSOT is `../../core/energy/domain.md`** (own 4). If a
   number in [`amd-ree-mineshaft-phes.md`](amd-ree-mineshaft-phes.md) diverges from `domain.md`,
   reconcile by updating either side — do NOT leave the divergence.
2. **Verb roster is contractual** (own 1). Do not rename `mineshaft` or move
...
3. **Group membership is contractual** (own 2). `mineshaft` belongs to group
   `mining`. Do not silently reassign.
...
| Task | Where |
|---|---|
| Read the spec | [`amd-ree-mineshaft-phes.md`](amd-ree-mineshaft-phes.md) |
| Tweak a number | First update `../../core/energy/domain.md`, then mirror here |
| Wire this verb | Add `verify_mineshaft.{hexa,py}`, register from `../../verify/energy_verify.py`, propagate wired count to spec.md/hexa.toml/cli |
...
Extracted from `n6-architecture/domains/energy/amd-ree-mineshaft-phes/` on 2026-05-06
(SHA `c0f1f570`). Moved from repo root into `module/mineshaft/` on 2026-05-07

# module/solar — solar verb (group: grid)
...
The primary spec is [`solar-architecture.md`](solar-architecture.md), a verbatim extraction
...
This verb is one of 3 verbs in the grid group.
...
| working kernel | ☐ TBD (helper present: `real-world-solar-calculator.hexa`) |
| selftest | sentinel only (`module/solar/` directory present) |

To wire this verb: add `verify_solar.{hexa,py}` that auto-derives ≥ 1
quantitative claim from `solar-architecture.md`, register the call from

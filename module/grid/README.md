# module/grid — grid verb (group: grid)
...
The primary spec is [`power-grid.md`](power-grid.md), a verbatim extraction
...
| selftest | sentinel only (`module/grid/` directory present) |

To wire this verb: add `verify_grid.{hexa,py}` that auto-derives ≥ 1
quantitative claim from `power-grid.md`, register the call from

# module/efficiency — AI-native handoff
...
- **Verb**: `efficiency` (group: `meta`)
- **Parent (canonical SSOT)**: `../../core/energy/domain.md` (numerical SSOT, own 4)
- **Upstream**: `n6-architecture/domains/energy/energy-efficiency/` @ `c0f1f570`
- **Status**: SPEC_FIRST (own 5) — markdown spec only at v1.0.0

## Edit invariants

1. **Numerical SSOT is `../../core/energy/domain.md`** (own 4). If a
   number in [`energy-efficiency.md`](energy-efficiency.md) diverges from `domain.md`,
   reconcile by updating either side — do NOT leave the divergence.
2. **Verb roster is contractual** (own 1). Do not rename `efficiency` or move
...
3. **Group membership is contractual** (own 2). `efficiency` belongs to group
   `meta`. Do not silently reassign.
...
| Task | Where |
|---|---|
| Read the spec | [`energy-efficiency.md`](energy-efficiency.md) |
| Tweak a number | First update `../../core/energy/domain.md`, then mirror here |
| Wire this verb | Add `verify_efficiency.{hexa,py}`, register from `../../verify/energy_verify.py`, propagate wired count to spec.md/hexa.toml/cli |
...
Extracted from `n6-architecture/domains/energy/energy-efficiency/` on 2026-05-06
(SHA `c0f1f570`). Moved from repo root into `module/efficiency/` on 2026-05-07

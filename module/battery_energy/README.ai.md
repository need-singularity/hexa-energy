# module/battery_energy — AI-native handoff
...
- **Verb**: `battery_energy` (group: `battery`)
- **Parent (canonical SSOT)**: `../../core/energy/domain.md` (numerical SSOT, own 4)
- **Upstream**: `n6-architecture/domains/energy/battery-energy/` @ `c0f1f570`
- **Status**: SPEC_FIRST (own 5) — markdown spec only at v1.0.0

## Edit invariants

1. **Numerical SSOT is `../../core/energy/domain.md`** (own 4). If a
   number in [`battery-energy.md`](battery-energy.md) diverges from `domain.md`,
   reconcile by updating either side — do NOT leave the divergence.
2. **Verb roster is contractual** (own 1). Do not rename `battery_energy` or move
...
3. **Group membership is contractual** (own 2). `battery_energy` belongs to group
...
| Task | Where |
|---|---|
| Read the spec | [`battery-energy.md`](battery-energy.md) |
| Tweak a number | First update `../../core/energy/domain.md`, then mirror here |
| Wire this verb | Add `verify_battery_energy.{hexa,py}`, register from `../../verify/energy_verify.py`, propagate wired count to spec.md/hexa.toml/cli |
...
Extracted from `n6-architecture/domains/energy/battery-energy/` on 2026-05-06
(SHA `c0f1f570`). Moved from repo root into `module/battery_energy/` on 2026-05-07

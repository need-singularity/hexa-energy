# module/dc_reactor — AI-native handoff
...
- **Verb**: `dc_reactor` (group: `nuclear`)
- **Parent (canonical SSOT)**: `../../core/energy/domain.md` (numerical SSOT, own 4)
- **Upstream**: `n6-architecture/domains/energy/datacenter-reactor/` @ `c0f1f570`
- **Status**: SPEC_FIRST (own 5) — markdown spec only at v1.0.0

## Edit invariants

1. **Numerical SSOT is `../../core/energy/domain.md`** (own 4). If a
   number in [`datacenter-reactor.md`](datacenter-reactor.md) diverges from `domain.md`,
   reconcile by updating either side — do NOT leave the divergence.
2. **Verb roster is contractual** (own 1). Do not rename `dc_reactor` or move
...
3. **Group membership is contractual** (own 2). `dc_reactor` belongs to group
   `nuclear`. Do not silently reassign.
...
| Task | Where |
|---|---|
| Read the spec | [`datacenter-reactor.md`](datacenter-reactor.md) |
| Tweak a number | First update `../../core/energy/domain.md`, then mirror here |
| Wire this verb | Add `verify_dc_reactor.{hexa,py}`, register from `../../verify/energy_verify.py`, propagate wired count to spec.md/hexa.toml/cli |
...
Extracted from `n6-architecture/domains/energy/datacenter-reactor/` on 2026-05-06
(SHA `c0f1f570`). Moved from repo root into `module/dc_reactor/` on 2026-05-07

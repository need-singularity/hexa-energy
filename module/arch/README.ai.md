...
3. **Group membership is contractual** (own 2). `arch` belongs to group
...
| Task | Where |
|---|---|
| Read the spec | [`energy-architecture.md`](energy-architecture.md) |
| Tweak a number | First update `../../core/energy/domain.md`, then mirror here |
| Wire this verb | Add `verify_arch.{hexa,py}`, register from `../../verify/energy_verify.py`, propagate wired count to spec.md/hexa.toml/cli |
...
Extracted from `n6-architecture/domains/energy/energy-architecture/` on 2026-05-06
(SHA `c0f1f570`). Moved from repo root into `module/arch/` on 2026-05-07
